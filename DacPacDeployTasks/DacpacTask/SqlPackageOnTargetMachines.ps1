...
function Get-SqlPackageOnTargetMachine
{
    ...
    if ($dacPath -eq $null)
    {
        try
        {
            $source = get-command SqlPackage.exe
            $dacPath = $source.Source
        }
        catch [System.Exception]
        {
            # Ignore
...
function Get-SqlPackageCmdArgs
{
    ...
    $sqlPkgCmdArgs = [string]::Format(' /Action:{0}', $action)
    ...
    $sqlPkgCmdArgs = [string]::Format('{0} /SourceFile:"{1}"', $sqlPkgCmdArgs, $dacpacFile)
    ...
    if ($action -eq 'Publish' -and !$blockOnDataLoss)
    {
        $sqlPkgCmdArgs = [string]::Format('{0} /p:BlockOnPossibleDataLoss=false /p:BackupDatabaseBeforeChanges=true', $sqlPkgCmdArgs)
    }

    # validate output file
    if ($action -in @('DeployReport', 'Publish') -and [System.IO.Path]::GetExtension($outputPath) -ne ".xml")
    {
        throw "Invalid output file [ $outputPath ] provided, should be an xml file"
    }
    if ($action -eq 'Script' -and [System.IO.Path]::GetExtension($outputPath) -ne ".sql")
    {
        throw "Invalid output file [ $outputPath ] provided, should be a xml file"
    }

    if ($action -eq 'Publish')
    {
        $sqlPkgCmdArgs = [string]::Format('{0} /DeployReportPath:"{1}"', $sqlPkgCmdArgs, $outputPath)
        $sqlPkgCmdArgs = [string]::Format('{0} /DeployScriptPath:"{1}"', $sqlPkgCmdArgs, (Replace-LastSubstring "$outputPath" .xml .sql))
    }
    else
    {
